...
)
{
    ...
    else
    {
        ...
        // Forward the consumed feed frames into the bounded Interfayce bridge.
...
This function fills the virtual microphone from the Interfayce bridge.
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        InterfayceAudioBridgeRead(m_pDmaBuffer + bufferOffset, runWrite);
...
This function forwards consumed feed audio to the virtual microphone bridge.
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        InterfayceAudioBridgeWrite(m_pDmaBuffer + bufferOffset, runWrite);
        bufferOffset = (bufferOffset + runWrite) % m_ulDmaBufferSize;
        ByteDisplacement -= runWrite;
    }
}
